...
the text. But since **brms** defaults to the logit link when `family =
binomial`, there was no need. The other important thing to notice is the
left hand side of the `formula` argument, `rvote | trials(1)`. **brms**
allows users to specify supplementary information about the criterion
variable using the `y | fun(<variable>)` syntax, where `y` is the
criterion, `fun()` is a generic stand-in for some function, and
`<variable>` is a generic stand-in for either a variable in the data set
or a value manually set by the user. In the case of logistic regression,
we use the `trials()` function, which tells **brms** how many trials
each value in the criterion `rvote` is out of. In the case of our data,
that value is always one. In that case, you can simply manually add in a
`1` within the `trials()` argument.
...
> model, \(X \beta + \text{error}\), to data \(y\) that take on the
> values 0 and 1. The coefficients in such a model could be interpreted
> as differences in probabilities, but we could have difficulties using
> the model for prediction–it would be possible to have predicted
...
\(\operatorname{Pr}(y_i = 1) = \operatorname{logit}^{-1}(X_i \beta)\).
Since \(\operatorname{Pr}(y_i = 1) = p_i\), we can also write the
generic model as
...
> \(\operatorname{logit}^{-1}(\alpha + \beta x) = 0.5\); see Figure
> 13.1. The slope of the curve–the derivative of the logistic
> function–is maximized at this point and attains the value
> \(\beta e^0 / (1 + e^0)^2 = \beta / 4\). Thus, \(\beta / 4\) is the
> maximum difference in \(\operatorname{Pr}(y = 1)\) corresponding to a
...
> \((p, 1 - p)\), then \(\frac{p}{1 - p}\) is called the *odds*…
> Dividing two odds, \(\frac{p_1}{1 - p_1} / \frac{p_2}{1 - p_2}\),
> gives an odds ratio. (pp. 220–221, *emphasis* in the original)
...
We returned a vector of posterior draws for \(a + b \times 5\). Here’s a
...
\(\operatorname{Pr}(y_\text{new} | x_\text{new})\). Here’s that that
...
\(\operatorname E (y_\text{new} | x_\text{new})\).
...
Whereas `pred` contains one *row* for each \(X^\text{new}\) value, the
other three objects contain one *column* for each \(X^\text{new}\)
value.

We might use `epred` to see how many of our posterior draws support the
hypothesis
\(\operatorname{Pr} (y_\text{new} | \text{income} = 5) > \operatorname{Pr} (y_\text{new} | \text{income} = 4)\).
...
of the latent variable \(z_i\) when the linear predictor, \(X_i \beta\),
is equal to -1.07.
...
allow one to directly estimate \(\sigma\) independently from \(\beta\).
The model

> has an essential indeterminacy when fit to binary data, and it is
> standard to resolve this by setting the variance parameter \(\sigma\)
> to a fixed value such as 1.6, chosen because the normal distribution
> with standard deviation 1.6 is close to the unit logistic
> distribution. (p. 228)
...
likely fall between 0 and 1. The \(\operatorname N(0.5, 0.5)\) prior
would be a good candidate in that the bulk of the probability mass would
...
As indicated in the text, this function will not run properly when `n
= 0`.
...
The two have nearly converged. “In this particular example, once \(n\)
is as large as 1000, the prior distribution doesn’t really make a
difference” (p. 230).
...
> with estimated parameters \(\hat \beta\) and apply it to
> \(n^\text{new}\) new data points with predictor matrix
> \(X^\text{new}\), thus yielding a vector of predictive probabilities
...
> 
> > \= \_{i = 1}<sup>{n</sup>} { \\begin{array}{@{}ll@{}} p\_i^ &  y^ = 1
> \\ (1 - p\_i^) &  y^ = 0. \\end{array} .
> 
...
\(\operatorname{logit}^{-1}(0.4 + x_i)\), where \(x\) can be either 0 or
1.
...
We can use `brms::predict()` to comupte the overall probability of
switching a well.
...
tibble(pred = pred) %>% 
  ...
  stat_halfeye(.width = .95) +
  scale_y_continuous(NULL, breaks = NULL) +
  xlab("overall probability of switching a well, given the bivariable model")
```
...
```
    ...
    ##  [1] tidybayes_2.3.1 brms_2.14.4     Rcpp_1.0.5      patchwork_1.1.0
    ...
    ##  [22] ggdist_2.4.0         haven_2.3.1          xfun_0.19           
